...
- Secret 和 Error 是相互独立的分布模块，默认均为 `pure`。选择 `combination` 后，会
  自动枚举 CBD/稀疏三元相加组合；`maxDistributionComponents` 默认 3，硬上限为 1–6。
  组合分布传给 estimator 时使用保守矩近似，并会在结果中明确标记警告；
...
参数搜索严格按 `n`、`q`、Secret/Error 分布的顺序枚举。启用 estimator 后，每个候选都会
保留 MATZOV/ADPS16 的经典与量子四组比较；第一个实测达到目标的候选立即返回。若范围
全部耗尽，则返回 `validation.search_status="no_feasible_candidate"`，并把候选标为最佳参考，
而不是达标推荐。
